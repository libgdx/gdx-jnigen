...
public class MSVCToolchain extends BaseToolchain {
    ...
    private String getVcVarsAllTarget () {
        boolean isHostARM = SharedLibraryLoader.architecture == Architecture.ARM;
        boolean isHostX86 = SharedLibraryLoader.architecture == Architecture.x86;

        boolean isTargetARM = target.architecture == Architecture.ARM;
        boolean isTargetX86 = target.architecture == Architecture.x86;

        boolean isHost32Bit = SharedLibraryLoader.bitness == Architecture.Bitness._32;
        boolean isHost64Bit = SharedLibraryLoader.bitness == Architecture.Bitness._64;
    ...
    private void compileC () {
        ...
        for (File buildCFiles : buildCFiles) {
            perFileArgs.clear();
            perFileArgs.addAll(args);

            perFileArgs.add(buildCFiles.getAbsolutePath());
            perFileArgs.add("/Fo:" + new File(buildDirectory, toObjectFile(buildCFiles.getName())).getAbsolutePath());
            perFileArgs.add("/c");
            perFileArgs.add("/EHsc");

            logger.info("Compiling C File {}", buildCFiles.getAbsolutePath());
